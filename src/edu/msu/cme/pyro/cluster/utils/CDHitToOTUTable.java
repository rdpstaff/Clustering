...
public class CDHitToOTUTable {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 4 && args.length != 5) {
            System.err.println("USAGE: CDHitToOTUTable <idmapping> <sample_mapping> <cdhit_file> <cluster_out> [minlen]");
            return;
        }
        ...
        BufferedReader cdhitReader = new BufferedReader(new InputStreamReader(new ProgressMonitorInputStream(null, "Reading " + args[2], new FileInputStream(args[2]))));
        ...
        List<String> sampleList = sampleMapping.getSampleList();
        out.print("Cluster");
        for (int index = 0; index < sampleList.size(); index++) {
            String sampleName = sampleList.get(index);
            sampleNameToIndex.put(sampleName, index);
            out.print("\t");
            out.print(sampleName);
        }
        out.println();

        int[] otuRow = new int[sampleList.size()];

        System.err.println("Beginning CDHit to otu table file conversion");
        ...
        try {
            while ((line = cdhitReader.readLine()) != null) {
                ...
                if (line.startsWith(">Cluster")) {
                    if (clusterId != -1) {
                        System.err.println("Processed cluster " + clusterId + " in " + (System.currentTimeMillis() - startTime) / 1000.0 + "s");
                        out.print(clusterId);
                        for (int index = 0; index < otuRow.length; index++) {
                            out.print("\t");
                            out.print(otuRow[index]);
                        }
                        out.println();
                    }

                    Arrays.fill(otuRow, 0);
                ...
                for (String seqid : idMapping.getIds(intSeqid)) {
                    otuRow[sampleNameToIndex.get(sampleMapping.getSampleById(seqid))]++;
                }
            }
        ...
        System.err.println((100 - cutoff) + ": " + clusterId + " clusters");
        cdhitReader.close();
        out.close();
    }
}
